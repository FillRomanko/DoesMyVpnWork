...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const versionControllerPlace = document.querySelector('[data-js="version-tag"]');
    fetch('data/version.json')
        .then(response => response.json())
        .then(data => {
            if (data && data.version) {
                versionControllerPlace.innerHTML = `Version: ${data.version}`;
            }
        })
        .catch(err => {
            console.error('Ошибка загрузки версии:', err);
            versionControllerPlace.innerHTML = `Version: unknown`;
        });
